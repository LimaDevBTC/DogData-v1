...
export function TransactionDetailsModal({ transaction, isOpen, onClose }: TransactionDetailsModalProps) {
  ...
  const getRankingDisplay = (holderRank: number | null | undefined, hasDog: boolean = true, isNewHolder: boolean = false) => {
    ...
    if (isNewHolder) {
      return <span className="text-orange-400 text-xs font-mono font-medium">NEW</span>;
    }
    ...
    return <span className="text-green-400 text-xs font-mono font-medium">#{holderRank}</span>;
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900/90 border border-gray-600/50 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        {/* Header */}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-gray-800/50 p-4 ">
            ...
            <div className="space-y-3">
              {transaction.all_inputs.map((sender, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-gray-700/50 ">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 bg-red-400 bg-opacity-20 -full flex items-center justify-center">
          ...
          <div className="bg-gray-800/50 p-4 ">
            ...
            <div className="space-y-3">
              {transaction.dog_outputs.map((receiver, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-gray-700/50 ">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 bg-green-400 bg-opacity-20 -full flex items-center justify-center">
          ...
          <div className="bg-gray-800/50 p-4 ">
            ...
            <div className="grid grid-cols-3 gap-4 text-center">
              <div>
                <div className="text-2xl font-bold text-red-400">-{transaction.all_inputs?.length || 0}</div>
                <div className="text-sm text-dog-gray-400">UTXOs Gastos</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-green-400">+{transaction.dog_outputs.length}</div>
                <div className="text-sm text-dog-gray-400">UTXOs Criados</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-orange-400">{transaction.dog_inputs.length}</div>

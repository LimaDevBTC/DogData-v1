...
interface Meta {
  ...
  capacidade: number; capacidadePorSetor: number[]
...
export default function CidadeClient() {
  ...
  const [d, setD] = useState<{ n: number; x: Int16Array; z: Int16Array; s: Uint8Array; c: Uint8Array; f: Uint16Array; g: Uint8Array } | null>(null)
  const [pinta, setPinta] = useState<'idade' | 'setor' | 'familia'>('idade')
  ...
  useEffect(() => {
    ...
    const raio = Math.max(0.45, esc * 7)
    for (let i = 0; i < d.n; i++) {
      ...
      g2.beginPath(); g2.arc(X, Y, raio, 0, Math.PI * 2); g2.fill()
    }
  }, [d, meta, pinta, zoom])

  const porSetor = useMemo(() => meta?.capacidadePorSetor ?? [], [meta])
        ...
        <aside className="w-[22rem] space-y-4 font-mono text-[11px]">
          <section>
            ...
            <dl className="mt-2 space-y-1">
              ...
              <L k="Capacidade do tecido" v={meta ? meta.capacidade.toLocaleString('pt-BR') : '…'} />
              <L k="Quartos" v={meta ? String(meta.quartos) : '…'} />
              <L k="Quarteirões" v={meta ? String(meta.quarteiroes) : '…'} />
              <L k="Lote" v={meta ? `${meta.lote_m2} m²` : '…'} />

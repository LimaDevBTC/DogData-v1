  ...
  try {
    ...
    for (const t of txs) {
      for (const r of arr(t.receivers)) {
        ...
        if (lab && Number(r.amount_dog) > 0) entrou.set(lab.entity, (entrou.get(lab.entity) || 0) + Number(r.amount_dog))
      }
      for (const s of arr(t.senders)) {
        const lab = rotulo.get(s.address)
        if (lab && Number(s.amount_dog) > 0) saiu.set(lab.entity, (saiu.get(lab.entity) || 0) + Number(s.amount_dog))
      }
    }
    ...
    if (maior && Number(maior.total_dog_moved) > 0) {
      out.push({
        id: `big-${maior.txid}`, kind: 'whale', at: maior.timestamp, source: 'índice',
    ...
    if (doado > 0) {
      out.push({
        id: 'donation-24h', kind: 'donation', at: new Date().toISOString(), source: 'índice',
    ...
    if (pend.length > 0) {
      ...
      out.push({
        id: 'mempool-now', kind: 'mempool', at: new Date().toISOString(), source: 'nosso nó',

...
export function registerTransactionTools(server) {
    ...
    // -----------------------------------------------------------------------
    server.tool("get_recent_transactions", "Get recent DOG token transactions from the live transaction feed. " +
        ...
        if (type) {
            transactions = transactions.filter((tx) => tx.type?.toLowerCase() === type.toLowerCase());
        }
    ...
    // -----------------------------------------------------------------------
    server.tool("search_transaction", "Search for a specific DOG transaction by transaction ID (txid). " +
        "Returns full transaction details if found.", {
        txid: z.string().describe("Transaction ID to search for"),
    }, async ({ txid }) => {
        const transactions = await loadTransactions();
        const tx = transactions.find((t) => t.txid?.toLowerCase() === txid.toLowerCase());
        if (!tx) {
            return toolResult({
                found: false,
                txid,
                message: "Transaction not found in the recent transaction feed. " +

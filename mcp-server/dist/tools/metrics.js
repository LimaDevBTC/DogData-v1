import { z } from "zod";
import { loadHolders, loadHoldersWithUtxoStats, DOG_TOTAL_SUPPLY, } from "../utils/data-loader.js";
...
export function registerMetricsTools(server) {
    ...
    // -----------------------------------------------------------------------
    server.tool("get_onchain_metrics", "Get on-chain metrics for DOG token including total holders, total UTXOs, " +
        "concentration metrics, UTXO age distribution, supply distribution, and MVRV ratio.", {}, async () => {
    ...
    // -----------------------------------------------------------------------
    server.tool("get_metrics_history", "Get historical metrics for DOG token over time. " +

# 📚 Documentação DOG DATA

## 📖 Índice

### Documentação Principal

1. **[PROJETO_COMPLETO.md](PROJETO_COMPLETO.md)** ⭐
   - Visão geral completa do projeto
   - Estrutura, scripts, APIs
   - Guia de uso e setup
   - **LEIA PRIMEIRO**

2. **[ON_CHAIN_METRICS_PLAN.md](ON_CHAIN_METRICS_PLAN.md)**
   - Plano completo da página de métricas
   - Indicadores UTXO-based
   - Roadmap de implementação

3. **[ANALISE_COMPLETA_DADOS_EXIBIDOS.md](ANALISE_COMPLETA_DADOS_EXIBIDOS.md)**
   - Análise detalhada de todas as 6 abas
   - Dados exibidos em cada página
   - Fontes de dados e APIs

4. **[UTXO_METRICS_SETUP.md](UTXO_METRICS_SETUP.md)**
   - Setup e uso do script de métricas
   - Como acumular dados históricos
   - Automação

---

## 📁 Arquivos Arquivados

Documentação antiga e obsoleta foi movida para `docs/archived/`:
- Planos antigos de métricas
- Análises obsoletas
- Documentação de features removidas

---

## 🔄 Atualizações

- **2026-01-07:** Limpeza completa e consolidação
- **2026-01-07:** Criação da página On-Chain Metrics
- **2026-01-07:** Otimizações de performance

---

**Para começar:** Leia [PROJETO_COMPLETO.md](PROJETO_COMPLETO.md)





...
| F1-T5 | **Validação de reconciliação** | ❌ FALHOU 2026-06-11 | Replay completou (953146, 344.5k txs, 571k creates/452.9k spends). **Instrumentação OK**: event log == `replay_utxo_set.json` EXATO (118.076==118.076, 0 diff). **MAS replay diverge da realidade**: 118.076 UTXOs vivos vs produção **246.247** (~metade!). Supply bate (Δ+0.024%), mas só 75.544 outpoints em comum. realized_cap 54M vs 264M (79% off), MVRV 392% off, profit 98% vs 22%. Causa provável: allocation do replay **sub-cria outputs** (1.66 outputs/tx) vs ord real. orphan spends=1 (ok). **Veredito: NÃO promover.** Tiebreaker pendente: `ord balances` (precisa parar scanner) confirma qual é a verdade |
...
### PIVOT A+B (decidido 2026-06-17) — substitui a abordagem replay/utxo_events

Replay abandonado (cascata de allocation, F1-T5). Estratégia: **A** forward-only acurado (já existe via `dog_metrics_history`) + **B** backfill da coorte sobrevivente (história real do último ano, viés de sobrevivência honesto pré-2026, coverage por ponto). Métricas de gasto (SOPR/CDD/realized-P&L) ficam forward-only.

| ID | Task | Status | DoD / nota |
|----|------|--------|-----------|
| B-1 | Fonte per-UTXO do set verdadeiro | ✅ | `data/dog_utxos_by_address.json` (produção, horário) já tem outpoint+amount+`ts` criação; preço via price_history |
| B-2 | Agregador da coorte sobrevivente | ✅ FEITO 2026-06-17 | `scripts/build_surviving_cohort_history.py` → 785 pts diários + `coverage_pct`. **Validado vs produção**: realized_cap 0.12%, profit 0.90%, LTH/STH 0.06-0.28%. (avg_age: meu supply-weighted = `weighted_avg_age_days` da produção; o 538.9 é a variante não-ponderada). Output `data/surviving_cohort_history.json` |
| B-3 | Servir B no frontend + wirar registry/charts | ✅ FEITO 2026-06-17 | B publicado em `public/data/surviving_cohort_history.json` (estático, 431KB). `onchain-chart-grid.tsx` mescla B (rico, 785 pts) + A (forward-only) por dia. `/charts` 200, todas as 14 chaves do registry com fonte. `<OnChainChart>` filtra range no cliente |
| B-4 | Cron diário do B (auto-refresh) | ✅ FEITO 2026-06-17 | `run_surviving_cohort_script()` no `automated_update.py` (passo 1b, após holders). Roda horário, ~1min, publica data/+public/data/ |
| A-1 | Forward-only acurado | 🟡 existe | `collect_metrics_history.ts` → `dog_metrics_history` (hourly). Garantir cobertura de métricas + granularidade intra-dia |
| coverage-UI | Disclaimer honesto de viés pré-100% | ⬜ | banda/indicador no chart quando coverage<~90% (deep history = coorte que segurou) |

### F2 — Aggregator + `metric_series` (legado — ver PIVOT A+B acima)
...
- **2026-06-11** — Relê concluída (19h, bloco 953146). Pós-conclusão automatizado rodou: scanner reativo + alcançou tip + `automated_update` de catch-up OK (holders 88.273→88.306, utxos→246.247 — **pipeline horária retomada ✅**).
- **2026-06-11** — 🔧 Incidente de produção resolvido: site congelado em ontem 18h porque o `git push` horário estourava (1) timeout 60s e (2) limite 100MB do GitHub — `data/utxo_events.jsonl` (325MB) foi varrido pelo `git add -A`. Fix: colapsado 38 commits, destrackeado+gitignorado o event log (mantido no disco) e spike json, push OK (39s). Dados frescos no ar. ✅ FEITO: gitignore + untrack de `data/dog_transactions/` (86k block files, commit e61578c09; origin agora com 0 block files, arquivos intactos no disco). Verificado que só o backend usa esses files (o site lê `public/data/dog_transactions.json`, único, que continua trackeado). Repo desinchado; pushes horários voltam a ser pequenos.
- **2026-06-11** — ❌ **F1-T5 FALHOU**: instrumentação perfeita (event log == replay_utxo_set EXATO), mas o **replay reconstrói metade dos UTXOs reais** (118k vs 246k) → cost basis errado (realized_cap 54M vs 264M). A abordagem "replay from-genesis" NÃO é fundação confiável como está. **Decisão estratégica necessária** (não promover). Hipótese: allocation sub-cria outputs. Próximo: tiebreaker `ord balances` + diagnóstico de allocation → decidir corrigir replay vs pivotar abordagem.
- **2026-06-17** — 🔬 **F1-T5 diagnóstico concluído** (`scripts/diagnose_replay_allocation.py`): causa raiz = **cascata de allocation**. A `allocate_dog_outputs` acerta 74% dos txids (38.744/52.685 match exato vs produção), mas quando erra, o utxo_set diverge → o teste "é DOG tx?" falha para txs que gastam o outpoint correto → **115.350 txs puladas inteiras** (155.389 outpoints = o grosso do gap 118k vs 246k). Produção (246k, do ord) é a verdade. **Veredito: abordagem replay-from-genesis é frágil; replicar runes do ord com precisão exata + cascata = inviável de confiar.** DECISÃO: pivotar (ver opções na conversa) — não investir em corrigir allocation. NOTA-CHAVE: produção já tem o primitivo acurado (cost basis per-outpoint do estado ATUAL via ord); o que falta é só história.
- **2026-06-17** — ✅ **Pivot A+B executado**: agregador da coorte sobrevivente (`build_surviving_cohort_history.py`) construído e **validado vs produção** (realized_cap 0.12%, profit 0.90%, LTH/STH 0.06-0.28%). 785 pts diários (abr/24→hoje) com coverage. Servido estático + mesclado A no `/charts` (B-3). Auto-refresh horário no automated_update (B-4). **`/charts` agora mostra história real.** Próximo: coverage-UI (disclaimer honesto) + expandir catálogo (HODL waves de `hodl_waves`, realized price, NUPL, cohort cost basis → Tier 1) + linkar `/charts` no nav. NOTA: produção JÁ calcula realized_cap acurado do estado ATUAL via lookup per-outpoint do ord (`get_utxo_age_and_timestamp`) — o que falta é só HISTÓRIA.

...
- **2026-06-11** — 🔧 Incidente de produção resolvido: site congelado em ontem 18h porque o `git push` horário estourava (1) timeout 60s e (2) limite 100MB do GitHub — `data/utxo_events.jsonl` (325MB) foi varrido pelo `git add -A`. Fix: colapsado 38 commits, destrackeado+gitignorado o event log (mantido no disco) e spike json, push OK (39s). Dados frescos no ar. PENDENTE (recomendado): gitignore de `data/dog_transactions/` (74k block files incham o repo; a mudança de `vout` da F1-T2 churnou todos). Causa do timeout: pushes horários futuros são pequenos de novo.

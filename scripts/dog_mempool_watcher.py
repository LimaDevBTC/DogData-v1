...
# o módulo irmão `runestone` tem que ser encontrável mesmo quando o serviço sobe
# de outro diretório
...
# ⚠️ O DECODIFICADOR MUDOU DE CASA e agora vive em scripts/runestone.py, porque o
# dog_block_scanner precisava dele e estava chamando o binário `ord` por
# transação para fazer a mesma coisa. Duas implementações do mesmo protocolo no
# mesmo repositório é promessa de divergência. A daqui era a boa e virou a
# canônica; a equivalência com o `ord` foi medida em 20.105 transações, com zero
# diferenças, antes de qualquer troca.
from runestone import (  # noqa: E402,F401
    KNOWN_EVEN_TAGS, TAG_BODY, TAG_CENOTAPH, TAG_FLAGS, TAG_HEIGHT_END,
    TAG_HEIGHT_START, TAG_MINT, TAG_OFFSET_END, TAG_OFFSET_START, TAG_POINTER,
    TAG_PREMINE, TAG_RUNE, TAG_AMOUNT, TAG_CAP, U128_MAX,
    _leb128, _runestone_payload, decode_runestone,
)

...
// os nove setores: um só degradê quente, de ferrugem a areia. Nove passos
// iguais, permutados para que setores vizinhos não caiam em tons vizinhos.
const SETOR_RAMPA = rampa(['#7C4F33', '#B98A55', '#EEDDB8'])
...
NIVEIS.forEach((v, k) => {
  ...
  corpo += `<path d="${caminho(ls, true)}" fill="${TERRA_RAMPA((k + 1) / NIVEIS.length)}" fill-rule="evenodd" `
    + `stroke="#8A7A62" stroke-width="0.45" stroke-opacity="0.28"/>\n`
})
...
for (const s of Object.keys(porSetor).map(Number).sort((a, b) => a - b)) {
  corpo += `<path d="${porSetor[s]}" fill="${corSetor(s)}"/>\n`
}
...
corpo += via(an7Path, 26, { cor: LARANJA, op: 1 })
corpo += `<path d="${an7Path.terra}" fill="none" stroke="${LARANJA_CLARO}" stroke-width="0.9" stroke-opacity="0.9"/>\n`
...
DEF.forEach((d, k) => {
  ...
  rotulos += TC(5050, rumo - 3, rumo + 3, `S0${k + 1}`, { tam: 8, esp: 3, cor: FUNDO, op: 0.75 })
})
corpo += rotulos

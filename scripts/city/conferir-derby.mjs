// DOG DERBY no navegador: o ciclo de carga real, medido nos dois perfis.
// Mesmo portão de chapas.mjs: UMA cidade, UM navegador, fechamento no finally.
//   node scripts/city/conferir-derby.mjs [--mobile]
//
// ⚠️ É ESTE PORTÃO QUE RESPONDE "QUEBRA NO CELULAR?", e nenhuma conta offline
// responde. Ele mede, no perfil mobile de verdade: quantas transferências saem,
// quantos triângulos e malhas ficam VISÍVEIS de perto, se alguma textura entrou,
// se o contexto WebGL caiu e qual tier o app escolheu. O verificador de sítio
// mede o chão; este mede o custo.
//
// ⚠️ E O CELULAR NÃO PODE BAIXAR O DETALHE. O contrato do loader é: base sempre,
// detalhe só desktop com qualidade acima de baixa e sem economia de dados. Se um
// dia alguém trocar a condição, o assert de "0 pedidos de detail no mobile"
...
const out = '/tmp/dogcity-derby/browser'
...
try {
  ...
  heartbeat = setInterval(() => {
    console.log(`Conferência ${perfil}: ${Math.round((Date.now() - started) / 1000)} s; pedidos derby=${requests.length}; erros=${errors.length}`)
  }, 20000)
  await page.addInitScript(() => {
    window.__derbyContextLost = 0
    document.addEventListener('webglcontextlost', () => window.__derbyContextLost++, true)
  })
  page.on('request', (r) => {
    if (/dog-derby/.test(r.url())) requests.push({ url: r.url(), ms: Date.now() - started })
  })
  page.on('console', (m) => {
    if (m.type() === 'error') errors.push(m.text())
    if (m.type() === 'warning') warnings.push(m.text())
    if (/derby/.test(m.text())) logs.push(m.text())
    if (/\[derby\]/.test(m.text())) console.log(m.text())
  })
  page.on('pageerror', (e) => errors.push(`pageerror: ${e.message}`))
  console.log(`Abrindo ${perfil}; aguarda o portão explícito da cidade`)
  await page.goto('http://localhost:3000/city?view=home&live=0&stats=1&quality=balanced', { waitUntil: 'domcontentloaded', timeout: 120000 })
  ...
  await page.waitForFunction(() => window.__plazaScene?.getObjectByName('DOG_DERBY')?.userData.derby?.base === 'ready', null, { timeout: 90000 })
  await page.waitForTimeout(2500)
  const sample = () => page.evaluate(() => {
    const g = window.__plazaScene.getObjectByName('DOG_DERBY')
    ...
    return { state: { ...g.userData.derby }, visible: g.visible, position: g.position.toArray(), triangles, meshes, textures,
      contextLost: window.__derbyContextLost, stats: window.__plazaStats,
      detailVisible: g.getObjectByName('DERBY_DETAIL')?.visible ?? false }
  })
  report.far = await sample()
  assert.equal(report.far.state.base, 'ready')
  assert.equal(requests.filter((r) => /detail/.test(r.url)).length, 0, 'não baixa detalhe na vista da praça')
  await page.screenshot({ path: `${out}/${perfil}-praca.jpg`, type: 'jpeg', quality: 85, timeout: 180000 })
  console.log('Praça conferida; aproximando pela opção DOG Derby do menu Places')
  await page.getByRole('button', { name: /^Places/ }).click()
  await page.getByRole('button', { name: /^DOG Derby/ }).click()
  if (!mobile) await page.waitForFunction(() => window.__plazaScene.getObjectByName('DOG_DERBY').userData.derby.detalhe === 'ready', null, { timeout: 90000 })
  ...
  await page.evaluate(() => {
    const g = window.__plazaScene.getObjectByName('DOG_DERBY'), T = window.__plazaTHREE
    const p = g.localToWorld(new T.Vector3(160, 60, 300))
    const target = g.localToWorld(new T.Vector3(0, 14, -40))
  ...
  assert(!errors.some((e) => /pageerror|derby|shader error|THREE.WebGLProgram/i.test(e)), 'erro novo da integração ou shader')
  report.ok = true
  ...
  if (page) {
    try { report.diagnostic = await page.evaluate(() => ({ ready: window.__plazaPronto,
      derby: window.__plazaScene?.getObjectByName('DOG_DERBY')?.userData,

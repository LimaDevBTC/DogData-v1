#!/usr/bin/env node
// ═══════════════════════════════════════════════════════════════════════════
// VARIANTE A, ATLAS LUNAR. Cópia de carta.mjs (mesmos dados, mesma geometria,
// mesmo enquadramento e mesmo cartucho) com a camada de ESTILO reescrita na
// direção do brief de 23/09 (marketing/CARTA-BRIEF.md): o espírito da v2
// levado ao máximo. Muda só o que segue, tudo dentro das camadas 1 a 5:
//   - a paleta dos quarteirões (úmber escuro, quase o fundo) e das três orlas
//     nomeadas, que ganham um tom mais claro e hierárquico (Orla Nobre >
//     Distrito Financeiro > Bay Shore, ecoando "a alça é o endereço mais
//     nobre" da memória do projeto);
//   - a rua deixa de ser só o vão negativo entre quarteirões e ganha um
//     traço CREME por cima do tecido (camada 2), como na v2; as três orlas
//     especiais continuam sem traço por lote, porque ali o lote é fundido de
//     propósito para o olho ler a fileira, não a divisa (aviso original);
//   - a curva de nível volta a atravessar a folha inteira, inclusive por
//     baixo da cidade: as mesmas linhas já calculadas na camada 1 são
//     redesenhadas como traço fino sobre os quarteirões, antes da malha
//     creme;
//   - a AN7 (a avenida da alça) volta a ser uma BANDA larga com degradê
//     radial, não um fio;
//   - os rótulos de lugar somem da maioria do programa (nada de nome de
//     planta industrial) e sobra só a lista curada de 14 do brief, com um
//     ponto laranja nos rótulos de ponto.
// Nada abaixo disto muda a leitura de dados, a geometria ou o cartucho.
// ═══════════════════════════════════════════════════════════════════════════
...
const AGUA = '#17404F', AGUA_FUNDA = '#0F2C38', COSTA = '#7FB0C4', AREIA = '#93805F'
const VIA = '#E9DECB'
...
// os seis setores comuns: quarteirão em úmber escuro, quase o fundo (§ direção
// A do brief). Sem degradê quente aqui de propósito: quem carrega a leitura
// da cidade agora é o TRAÇO creme da rua (camada 2), não o tom do lote.
const UMBER_RAMPA = rampa(['#161210', '#1C1712', '#241D17'])
// as três orlas nomeadas ganham um tom mais claro e hierárquico: a alça
// (Orla Nobre) é o endereço mais nobre da cidade (memória do projeto),
// depois o Distrito Financeiro, depois a Orla da Baía.
...
let nPontosRelevo = 0
// guarda as mesmas linhas de nível para redesenhar por cima do tecido depois
// (direção A: "curvas de nível fortes na folha inteira, inclusive sob a
// cidade"). É a MESMA geometria da camada 1, só desenhada de novo mais tarde.
const linhasRelevo = []
NIVEIS.forEach((v, k) => {
  ...
  corpo += `<path d="${caminho(ls, true)}" fill="${TERRA_RAMPA((k + 1) / NIVEIS.length)}" fill-rule="evenodd" `
    + `stroke="${BRUMA}" stroke-width="0.75" stroke-opacity="0.5"/>\n`
})
...
// 1) o preenchimento, translúcido: a curva de nível de baixo ainda respira
const ORDEM_SETOR = Object.keys(porSetor).map(Number).sort((a, b) => a - b)
for (const s of ORDEM_SETOR) {
  corpo += `<path d="${porSetor[s]}" fill="${corSetor(s)}" fill-opacity="0.88"/>\n`
}
...
for (const ls of linhasRelevo) {
  corpo += `<path d="${caminho(ls, true)}" fill="none" stroke="${BRUMA}" stroke-width="0.65" stroke-opacity="0.3"/>\n`
}
// 3) a rua como LINHA creme fina e densa, por cima de tudo (direção A). Nas
// três orlas especiais (s >= 7) o traço fica de fora: o lote ali é fundido
// de propósito para o olho ler a fileira, não a divisa (aviso da camada 2).
for (const s of ORDEM_SETOR) {
  if (s >= 7) continue
  corpo += `<path d="${porSetor[s]}" fill="none" stroke="${RUA_LINHA}" stroke-width="0.62" stroke-opacity="0.55" stroke-linejoin="round"/>\n`
}
...
// direção A, "poucos rótulos, grandes": só os 14 nomes do brief levam texto.
// Nenhuma planta industrial, nenhum parque genérico. CAMPUS fica de fora da
// lista (o brief não pede) mas mantém a hachura de âncora sem nome.
...
for (const p of cidade.programa) {
  ...
  // ponto laranja em todo rótulo de ponto (direção A), do mesmo jeito que
  // Satoshi Plaza já marca o centro da cidade
...
// Direção A: volta a ser uma BANDA larga com degradê (como na v2), não um
// fio. Como a AN7 é um círculo perfeito, um degradê RADIAL centrado na
// própria Praça varia só com o raio, exatamente como a banda pede: escuro
// nas duas bordas, claro no meio.
const BANDA_AN7 = 70   // m, largura desenhada (os 26 originais viravam fio)
...
defs += `<radialGradient id="gradArco" gradientUnits="userSpaceOnUse" cx="${cx}" cy="${cx}" r="${rOut.toFixed(2)}">`
  + `<stop offset="${fIn.toFixed(4)}" stop-color="${LARANJA}"/>`
  + `<stop offset="${((fIn + 1) / 2).toFixed(4)}" stop-color="${LARANJA_CLARO}"/>`
  + `<stop offset="1" stop-color="${LARANJA}"/></radialGradient>\n`
corpo += via(an7Path, BANDA_AN7, { cor: 'url(#gradArco)', op: 1 })
corpo += `<path d="${an7Path.terra}" fill="none" stroke="${LARANJA_CLARO}" stroke-width="1.1" stroke-opacity="0.85"/>\n`
...
DEF.forEach((d, k) => {
  ...
  // direção A: o quarteirão ficou úmber escuro, quase o fundo; o código de
  // setor em FUNDO (pensado como recorte sobre um tom claro) sumiria. Sobe
  // para APAGADO, ainda discreto, mas legível sobre o tecido novo.
  rotulos += TC(5050, rumo - 3, rumo + 3, `S0${k + 1}`, { tam: 8, esp: 3, cor: APAGADO, op: 0.6 })
})
corpo += rotulos
...
const SIMB = [
  ...
  ['AN7 · THE SPIT AVENUE', (x, y) => `<line x1="${x}" y1="${y - 3}" x2="${x + 14}" y2="${y - 3}" stroke="url(#gradArco)" stroke-width="4.5"/>`],
...
${corpo}${mob}</svg>`
const SCRATCH_CARTAS = '/tmp/claude-1000/-home-bitmax-Projects-bitcoin-fullstack/8c325219-1e13-44e6-9724-1b09f71f34c8/scratchpad/cartas'
const SVG_SAIDA = resolve(RAIZ, arg('svg', `${SCRATCH_CARTAS}/varA.svg`))
...
// variante A: nada vai para public/, tudo fica no scratchpad da sessão
const saidaWebp = resolve(RAIZ, arg('webp', `${SCRATCH_CARTAS}/varA-1600.webp`))
const saidaPng1 = resolve(arg('png1600', `${SCRATCH_CARTAS}/varA-1600.png`))
const saidaPng2 = resolve(RAIZ, arg('png3200', `${SCRATCH_CARTAS}/varA-3200.png`))

...
const EST: Modulo = { i: 11, nr: 3, j: 46, ns: 3 }
...
const perto = res.filter((o) => o.dEst < 1100)
console.log(`blocos que passam nas TRES familias de via + agua: ${res.length}`)
console.log('ordenados por FOLGA, so os que ficam a menos de 1.100 m do estadio:')
...
const atual = centro({ i: 8, nr: 3, j: 46, ns: 3 })
const va = folgaDeVia(atual.x, atual.z, atual.r)
console.log(`\nSITIO ATUAL i8 j46: raio ${atual.r.toFixed(0)}, folga ${va.folga.toFixed(0)} m ate ${va.quem}`)
const ve = folgaDeVia(cEst.x, cEst.z, cEst.r)
console.log(`ESTADIO      i11 j46: raio ${cEst.r.toFixed(0)}, folga ${ve.folga.toFixed(0)} m ate ${ve.quem}`)

const NOVO: Modulo = { i: 11, nr: 3, j: 52, ns: 2 }
const cn = centro(NOVO); const vn = folgaDeVia(cn.x, cn.z, cn.r)
console.log(`\nESCOLHIDO i11 nr3 j52 ns2`)
...
console.log(`  distancia ao estadio: ${Math.hypot(cn.x-cEst.x, cn.z-cEst.z).toFixed(0)} m`)
